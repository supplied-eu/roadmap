...
function renderMyDay(){
  ...
  if(GOOGLE_DATA && GOOGLE_DATA.email_priorities && GOOGLE_DATA.email_priorities.length){
    const prioSec = document.createElement(“div”);
    prioSec.className = “myday-section”;
    if(!showInbox){ prioSec.style.display=”none”; }
    const ACTION_META = {
      reply:    { label:”REPLY”,    color:”#ef4444”, icon:”↩” },
      followup: { label:”FOLLOW UP”,color:”#f59e0b”, icon:”⏰” },
      alert:    { label:”ALERT”,    color:”#8b5cf6”, icon:”⚠” },
    };
    prioSec.innerHTML = \`<div class=”myday-section-hdr”><span>🚨 EMAIL PRIORITIES</span><span class=”myday-section-count”>\${GOOGLE_DATA.email_priorities.length}</span></div>\`;
    for(const p of GOOGLE_DATA.email_priorities){
      const m = ACTION_META[p.action] || ACTION_META.reply;
      const row = document.createElement(“div”);
      row.className = “myday-item”;
      if(p.url){ row.style.cursor=”pointer”; row.addEventListener(“click”,()=>window.open(p.url,”_blank”)); }
      row.innerHTML = \`
        <span class=”myday-source” style=”background:\${m.color};color:#fff;min-width:52px;text-align:center;font-size:9px”>\${m.icon} \${m.label}</span>
        <span class=”myday-item-title”>\${p.subject}</span>
        <span class=”myday-item-status” style=”font-style:italic;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:260px”>\${p.reason}</span>
      \`;
      prioSec.appendChild(row);
    }
    main.appendChild(prioSec);
  }
