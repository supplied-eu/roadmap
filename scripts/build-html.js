...
.row-name-link { text-decoration: none; color: inherit; cursor: pointer; }
.row-name-link:hover { text-decoration: underline; color: var(--accent, #3b82f6); }
.row-badge {
...
/* ── Customer Project Drag ────────────────────────────────────────────── */
.cust-proj-wrap{transition:opacity .15s;}
.cust-proj-wrap.dragging{opacity:.35;}
.cust-proj-wrap.drag-over{box-shadow:inset 0 2px 0 var(--accent);}
/* ── Initiative Tree ──────────────────────────────────────────────────── */
...
.ini-name-link{text-decoration:none;color:inherit;cursor:pointer;}.ini-name-link:hover{text-decoration:underline;color:var(--accent,#3b82f6);}
...
.ini-proj-name-link{text-decoration:none;color:inherit;cursor:pointer;}.ini-proj-name-link:hover{text-decoration:underline;color:var(--accent,#3b82f6);}
...
.acct-detail { display:none; background:var(--bg); border:1px solid var(--border); border-radius:8px; margin-top:-6px; margin-bottom:14px; overflow:hidden; }
.acct-detail.open { display:block; }
.acct-detail-tabs { display:flex; border-bottom:1px solid var(--border); background:var(--surface); }
.acct-detail-tab { font-size:10px; padding:8px 14px; border:none; background:transparent; color:var(--text-muted); cursor:pointer; letter-spacing:.3px; border-bottom:2px solid transparent; transition:all .15s; }
.acct-detail-tab.active { color:var(--accent); border-bottom-color:var(--accent); }
.acct-detail-body { padding:14px; min-height:120px; }
/* Account plan */
.acct-plan-area { width:100%; min-height:80px; background:var(--surface2); border:1px solid var(--border); border-radius:6px; padding:10px; font-size:11px; color:var(--text); font-family:inherit; resize:vertical; outline:none; line-height:1.6; }
.acct-plan-area:focus { border-color:var(--accent); }
.acct-plan-save { font-size:9px; padding:4px 12px; border-radius:4px; border:1px solid var(--accent); background:var(--accent); color:#fff; cursor:pointer; margin-top:6px; float:right; }
.acct-plan-hint { font-size:9px; color:var(--text-dim); margin-bottom:6px; }
/* Account task list */
.acct-task-row { display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid rgba(255,255,255,.04); font-size:10px; }
.acct-task-row.done { opacity:.4; }
.acct-task-check { accent-color:var(--accent); cursor:pointer; }
.acct-task-title { flex:1; color:var(--text); }
.acct-task-title.done { text-decoration:line-through; }
.acct-task-add-row { display:flex; gap:6px; margin-top:8px; }
.acct-task-add-inp { flex:1; background:var(--surface2); border:1px solid var(--border); border-radius:4px; padding:5px 8px; font-size:10px; color:var(--text); outline:none; }
.acct-task-add-inp:focus { border-color:var(--accent); }
.acct-task-add-btn { font-size:9px; padding:4px 10px; border-radius:4px; border:1px solid var(--border); background:var(--surface2); color:var(--accent); cursor:pointer; }
/* Timeline */
.acct-timeline-item { display:flex; gap:10px; padding:8px 0; border-bottom:1px solid rgba(255,255,255,.04); }
.acct-timeline-dot { width:8px; height:8px; border-radius:50%; flex-shrink:0; margin-top:4px; }
.acct-timeline-content { flex:1; }
.acct-timeline-title { font-size:10px; color:var(--text); font-weight:500; }
.acct-timeline-meta { font-size:9px; color:var(--text-dim); margin-top:2px; }
.acct-timeline-body { font-size:10px; color:var(--text-muted); margin-top:3px; line-height:1.5; }
/* KPI chips in account */
.acct-kpi-row { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:10px; }
.acct-kpi-chip { font-size:10px; padding:4px 10px; border-radius:6px; background:var(--surface2); border:1px solid var(--border); }
.acct-kpi-chip-label { font-size:8px; color:var(--text-dim); text-transform:uppercase; letter-spacing:.5px; display:block; }
.acct-kpi-chip-value { font-size:13px; font-weight:600; color:var(--text); }
...
/* Hide all dashboard content until user logs in */
body.locked .header, body.locked .tab-panel { visibility:hidden; pointer-events:none; filter:blur(8px); }
body.locked .user-select-overlay.open { visibility:visible; pointer-events:auto; filter:none; }
.user-select-box {
...
<div class="user-select-overlay" id="user-select-overlay">
  <div class="user-select-box">
    <div class="user-select-title">🔒 Log in to Dashboard</div>
    <div class="user-select-sub">Select your name to access your personalised view</div>
...
<div class="header">
  ...
  <div class="header-right">
    ...
    <button class="refresh-btn" id="logout-btn" onclick="logOut()" title="Log out" style="padding:3px 7px;font-size:9px;display:none;">⏻</button>
...
function setCurrentUser(name){
  ...
  applyAccessControl();
  // Re-run full init now that user is set (loads data scoped to user)
  init();
}
function applyAccessControl(){
  ...
  if(kpiBtn) kpiBtn.style.display=admin?"":"none";
  // Accounts tab — admin only (contains all customer data)
  const acctBtn=document.getElementById("tab-accounts-btn");
  if(acctBtn) acctBtn.style.display=admin?"":"none";
  // Person bar in My Day — admin only
  const pbar=document.getElementById("myday-person-bar");
  if(pbar) pbar.style.display=admin?"":"none";
  ...
  if(logoutBtn) logoutBtn.style.display=CURRENT_USER?"":"none";
  // If non-admin is on a restricted tab, redirect to myday
  if(!admin && (currentTab==="kpis"||currentTab==="accounts")){
    switchTab("myday");
  }
}
...
function logOut(){
  CURRENT_USER=null;
  localStorage.removeItem("supplied_current_user");
  GOOGLE_DATA=null;
...
function switchTab(tab){
  // Guard: non-admins cannot access restricted tabs
  if((tab==="kpis"||tab==="accounts") && !isAdmin()) return;
  currentTab = tab;
...
function makeGanttRow({indent,label,status,color,start,end,url,hasChildren,isExpanded,isIni,onClick,barData,barClickable,assignee,priority,overdueFlag}){
  ...
  if(url){
    const nm=document.createElement("a"); nm.className="row-name row-name-link"; nm.textContent=label; nm.title=label; nm.href=url; nm.target="_blank"; nm.rel="noopener";
    nm.addEventListener("click",function(ev){ ev.stopPropagation(); }); lc.appendChild(nm);
  } else {
...
function renderCustomerCommitments(container){
  ...
  // Only show projects that have at least one open issue; hide if all issues are done/completed
  ...
  const activeProjs=(custIni.projects||[]).filter(p=>{
    ...
    // Must have at least one open (non-done, non-canceled) issue
  ...
  function rebuildCust(){
    ...
    for(let idx=0;idx<orderedProjs.length;idx++){
      ...
      // Only show issues that are actively being worked (exclude Backlog, Done, Canceled, etc.)
      const activeIss=(proj.issues||[]).filter(i=>!INACTIVE.has(i.status||"")&&!DONE_STATES.has(i.status||""))
      ...
      // ── Draggable project wrapper ───────────────────────────────────────
      const projWrap=document.createElement("div"); projWrap.draggable=true; projWrap.className="cust-proj-wrap";
      projWrap.addEventListener("dragstart",function(e){ dragSrcIdx=idx; projWrap.classList.add("dragging"); e.dataTransfer.effectAllowed="move"; });
      projWrap.addEventListener("dragend",function(){ projWrap.classList.remove("dragging"); wrapper.querySelectorAll(".drag-over").forEach(el=>el.classList.remove("drag-over")); });
      projWrap.addEventListener("dragover",function(e){ e.preventDefault(); e.dataTransfer.dropEffect="move"; if(dragSrcIdx!==idx) projWrap.classList.add("drag-over"); });
      projWrap.addEventListener("dragleave",function(){ projWrap.classList.remove("drag-over"); });
      projWrap.addEventListener("drop",function(e){ e.preventDefault(); projWrap.classList.remove("drag-over"); if(dragSrcIdx===null||dragSrcIdx===idx) return; const moved=orderedProjs.splice(dragSrcIdx,1)[0]; orderedProjs.splice(idx,0,moved); dragSrcIdx=null; saveCustOrder(orderedProjs); rebuildCust(); });
...
function renderInitiativeTree(container){
  ...
  function rebuildTree(){
    ...
    for(let idx=0;idx<orderedInis.length;idx++){
      ...
      let name; if(ini.url){ name=document.createElement("a"); name.className="ini-name ini-name-link"; name.textContent=ini.name; name.href=ini.url; name.target="_blank"; name.rel="noopener"; name.addEventListener("click",function(ev){ ev.stopPropagation(); }); } else { name=document.createElement("span"); name.className="ini-name"; name.textContent=ini.name; }
      ...
      if(iniExp){
        for(const proj of (ini.projects||[])){
          ...
          let pName; if(proj.url){ pName=document.createElement("a"); pName.className="ini-proj-name ini-proj-name-link"; pName.textContent=proj.name; pName.href=proj.url; pName.target="_blank"; pName.rel="noopener"; pName.addEventListener("click",function(ev){ ev.stopPropagation(); }); } else { pName=document.createElement("span"); pName.className="ini-proj-name"; pName.textContent=proj.name; }
...
function renderAccountManagement(){
  ...
  if(!container) return;
  container.innerHTML="";
  let companies=(HS_DATA&&HS_DATA.companies)||[];
  // Fallback: if no companies from HubSpot lifecycle stage filter, derive from deals
  if(!companies.length && HS_DATA && HS_DATA.deals && HS_DATA.deals.length){
    const dealCos=new Map();
    for(const d of HS_DATA.deals){
      const dn=d.name||"";
      const cn=d.companyName||dn.split(/\\s*[-\u2013\u2014]\\s*/)[0].trim();
      if(!cn) continue;
      if(!dealCos.has(cn)){
        dealCos.set(cn,{
          id:"derived-"+cn.replace(/\\s+/g,"-").toLowerCase(),
          name:cn, domain:null, ownerId:d.ownerId||null,
          ownerName:d.ownerName||null, contacts:0,
          openDeals:0, recentDealAmount:null, daysSinceContact:null,
          lastContacted:null, notes:[], createdAt:null,
        });
      }
      const co=dealCos.get(cn);
      if(!isDoneStage(d.stageLabel)) co.openDeals++;
      if(d.amount&&(!co.recentDealAmount||d.amount>co.recentDealAmount)) co.recentDealAmount=d.amount;
      if(d.modifiedDate){
        const dt=(d.modifiedDate||"").split("T")[0];
        if(!co.lastContacted||dt>co.lastContacted){ co.lastContacted=dt; co.daysSinceContact=Math.floor((Date.now()-new Date(dt))/86400000); }
      }
    }
    companies=[...dealCos.values()];
  }
  if(!companies.length){
    container.innerHTML=\`<div style="padding:60px;text-align:center;color:var(--text-dim);font-size:12px;">No customer data found.<br><br>Make sure companies are marked with lifecycle stage <strong>Customer</strong> in HubSpot, or check that deals have been loaded.</div>\`;
    return;
  }
  const portalId=HS_DATA&&HS_DATA.portalId||"";
  const cur=HS_DATA&&HS_DATA.currency||"\u20AC";
  // Resolve owner names from owner IDs
  const ownerMap={};
  if(HS_DATA&&HS_DATA.owners) HS_DATA.owners.forEach(o=>{ ownerMap[o.id]=o.name; });
  for(const c of companies){ if(!c.ownerName&&c.ownerId&&ownerMap[c.ownerId]) c.ownerName=ownerMap[c.ownerId]; }
  // Local storage keys for plans and tasks
  const ACCT_PLANS_KEY="supplied_acct_plans";
  const ACCT_TASKS_KEY="supplied_acct_tasks";
  const acctPlans=JSON.parse(localStorage.getItem(ACCT_PLANS_KEY)||"{}");
  const acctTasks=JSON.parse(localStorage.getItem(ACCT_TASKS_KEY)||"{}");
  function savePlans(){ localStorage.setItem(ACCT_PLANS_KEY, JSON.stringify(acctPlans)); }
  function saveTasks(){ localStorage.setItem(ACCT_TASKS_KEY, JSON.stringify(acctTasks)); }

  // Enrich companies with related data
  const enriched=companies.map(c=>{
    const h=computeHealthScore(c);
    // Find related deals
    const relDeals=(HS_DATA.deals||[]).filter(d=>{
      if(d.companyId&&d.companyId===c.id) return true;
      const dn=(d.name||"").toLowerCase(), cn=(c.name||"").toLowerCase();
      return cn.length>3&&dn.includes(cn);
    });
    // Find related Linear issues (match by company name in initiative/project names)
    const cn=(c.name||"").toLowerCase();
    const relIssues=[];
    if(GANTT_DATA&&cn.length>3){
      for(const ini of (GANTT_DATA.initiatives||[])){
        const iniMatch=(ini.name||"").toLowerCase().includes(cn);
        for(const proj of (ini.projects||[])){
          const projMatch=iniMatch||(proj.name||"").toLowerCase().includes(cn);
          if(projMatch){
            for(const iss of (proj.issues||[])){
              if(isActive(iss.status)) relIssues.push({...iss,_proj:proj.name,_ini:ini.name});
            }
          }
        }
      }
    ...
    const closedDeals=relDeals.filter(d=>isDoneStage(d.stageLabel)&&(d.stageLabel||"").toLowerCase().includes("won"));
    const totalRevenue=closedDeals.reduce((s,d)=>s+(d.amount||0),0);
    const openPipeline=relDeals.filter(d=>!isDoneStage(d.stageLabel)).reduce((s,d)=>s+(d.amount||0),0);
    return {...c,health:h,nextSteps:getNextSteps(c,h),relDeals,relIssues,totalRevenue,openPipeline,closedDeals};
  });
  ...
  // Summary KPIs
  const kpiRow=document.createElement("div"); kpiRow.className="acct-kpi-row";
  const totalRev=enriched.reduce((s,c)=>s+c.totalRevenue,0);
  const totalPipe=enriched.reduce((s,c)=>s+c.openPipeline,0);
  const healthyCt=enriched.filter(c=>c.health.cls==="green").length;
  const atRiskCt=enriched.filter(c=>c.health.cls==="amber").length;
  const needsAttCt=enriched.filter(c=>c.health.cls==="red").length;
  [{l:"Total Customers",v:enriched.length},{l:"Healthy",v:healthyCt,c:"#10b981"},{l:"At Risk",v:atRiskCt,c:"#f59e0b"},{l:"Needs Attention",v:needsAttCt,c:"#ef4444"},{l:"Total Revenue",v:cur+totalRev.toLocaleString()},{l:"Open Pipeline",v:cur+totalPipe.toLocaleString()}].forEach(k=>{
    const chip=document.createElement("div"); chip.className="acct-kpi-chip";
    chip.innerHTML=\`<span class="acct-kpi-chip-label">\${k.l}</span><span class="acct-kpi-chip-value" style="\${k.c?"color:"+k.c:""}">\${k.v}</span>\`;
    kpiRow.appendChild(chip);
  });
  container.appendChild(kpiRow);

  const listEl=document.createElement("div"); listEl.id="acct-list"; container.appendChild(listEl);
  ...
  function renderCards(){
    ...
    for(const c of visible){
      // ── Card (summary row) ──────────────────────────────────────────
      const card=document.createElement("div"); card.className="acct-card"; card.style.cursor="pointer";
      // Owner pill
      if(c.ownerName){
      ...
      const contactTxt=dsc===null?"Never contacted":dsc===0?"Today":dsc===1?"Yesterday":dsc+"d ago";
      [{txt:contactTxt,cls:contactCls},{txt:c.contacts+" contact"+(c.contacts!==1?"s":"")},{txt:c.openDeals+" deal"+(c.openDeals!==1?"s":""),cls:c.openDeals>0?"ok":""},
       {txt:c.relIssues.length+" issue"+(c.relIssues.length!==1?"s":"")},
       ...(c.totalRevenue?[{txt:cur+c.totalRevenue.toLocaleString()+" revenue",cls:"ok"}]:[]),
       ...(c.openPipeline?[{txt:cur+c.openPipeline.toLocaleString()+" pipeline"}]:[])
      ].forEach(m=>{
      ...
      // Next step teaser
      const ns=document.createElement("div"); ns.style.cssText="font-size:10px;color:var(--accent);margin-top:4px;";
      ns.textContent="\u2192 "+c.nextSteps;
      card.appendChild(ns);

      // ── Detail panel (hidden by default) ────────────────────────────
      const detail=document.createElement("div"); detail.className="acct-detail";

      // Tab bar
      const detailTabs=document.createElement("div"); detailTabs.className="acct-detail-tabs";
      const tabDefs=[{key:"plan",label:"\uD83D\uDCCB Plan"},{key:"tasks",label:"\u2611\uFE0F Tasks"},{key:"timeline",label:"\uD83D\uDD70 Timeline"},{key:"deals",label:"\uD83D\uDCB0 Deals"},{key:"issues",label:"\uD83D\uDD27 Issues"}];
      const detailBody=document.createElement("div"); detailBody.className="acct-detail-body";
      let activeDetailTab="plan";

      function renderDetailTab(tabKey){
        activeDetailTab=tabKey;
        detailBody.innerHTML="";
        detailTabs.querySelectorAll(".acct-detail-tab").forEach(t=>t.classList.toggle("active",t.dataset.tab===tabKey));

        if(tabKey==="plan"){
          // Editable plan
          const hint=document.createElement("div"); hint.className="acct-plan-hint";
          hint.textContent="Write your account plan, goals, and strategy for "+c.name;
          detailBody.appendChild(hint);
          const ta=document.createElement("textarea"); ta.className="acct-plan-area";
          ta.value=acctPlans[c.id]||"";
          ta.placeholder="e.g. Q2 goals: expand to 3 new departments, schedule QBR, renew contract by June...";
          detailBody.appendChild(ta);
          const saveBtn=document.createElement("button"); saveBtn.className="acct-plan-save"; saveBtn.textContent="Save Plan";
          saveBtn.addEventListener("click",()=>{ acctPlans[c.id]=ta.value; savePlans(); saveBtn.textContent="\u2705 Saved"; setTimeout(()=>saveBtn.textContent="Save Plan",1500); });
          detailBody.appendChild(saveBtn);
          // Clear float
          const cl=document.createElement("div"); cl.style.clear="both"; detailBody.appendChild(cl);
        }

        else if(tabKey==="tasks"){
          // Per-account task list
          if(!acctTasks[c.id]) acctTasks[c.id]=[];
          const tasks=acctTasks[c.id];
          function drawTasks(){
            detailBody.innerHTML="";
            if(!tasks.length){
              const em=document.createElement("div"); em.style.cssText="font-size:10px;color:var(--text-dim);padding:10px 0;";
              em.textContent="No tasks yet. Add one below."; detailBody.appendChild(em);
            }
            for(const [idx,t] of tasks.entries()){
              const row=document.createElement("div"); row.className="acct-task-row"+(t.done?" done":"");
              const cb=document.createElement("input"); cb.type="checkbox"; cb.checked=!!t.done; cb.className="acct-task-check";
              cb.addEventListener("change",()=>{ t.done=cb.checked; saveTasks(); drawTasks(); });
              const titleEl=document.createElement("span"); titleEl.className="acct-task-title"+(t.done?" done":""); titleEl.textContent=t.title;
              const del=document.createElement("button"); del.style.cssText="font-size:9px;color:var(--text-dim);background:transparent;border:none;cursor:pointer;padding:2px 4px;"; del.textContent="\u2715";
              del.addEventListener("click",()=>{ tasks.splice(idx,1); saveTasks(); drawTasks(); });
              row.appendChild(cb); row.appendChild(titleEl); row.appendChild(del); detailBody.appendChild(row);
            }
            // Add task input
            const addRow=document.createElement("div"); addRow.className="acct-task-add-row";
            const inp=document.createElement("input"); inp.className="acct-task-add-inp"; inp.placeholder="Add a task for "+c.name+"\u2026";
            const addBtn=document.createElement("button"); addBtn.className="acct-task-add-btn"; addBtn.textContent="+ Add";
            function doAdd(){ const v=(inp.value||"").trim(); if(!v) return; tasks.unshift({title:v,done:false,created:TODAY_STR}); saveTasks(); drawTasks(); }
            addBtn.addEventListener("click",doAdd);
            inp.addEventListener("keydown",e=>{ if(e.key==="Enter"){ e.preventDefault(); doAdd(); }});
            addRow.appendChild(inp); addRow.appendChild(addBtn); detailBody.appendChild(addRow);
          }
          drawTasks();
        }

        else if(tabKey==="timeline"){
          // Timeline: notes + recent deal activity
          const events=[];
          if(c.notes) for(const n of c.notes) events.push({type:"note",date:n.date||"",title:"Note",body:n.body||"",color:"#3b82f6"});
          for(const d of c.relDeals){
            if(d.createDate) events.push({type:"deal",date:d.createDate.split("T")[0],title:"Deal created: "+d.name,body:d.amount?cur+Number(d.amount).toLocaleString():"",color:"#10b981"});
            if(d.modifiedDate&&d.modifiedDate!==d.createDate) events.push({type:"deal",date:d.modifiedDate.split("T")[0],title:"Deal updated: "+d.name,body:"Stage: "+(d.stageLabel||d.stage||""),color:"#8b5cf6"});
          }
          events.sort((a,b)=>(b.date||"").localeCompare(a.date||""));
          if(!events.length){
            detailBody.innerHTML='<div style="font-size:10px;color:var(--text-dim);padding:10px 0;">No activity recorded yet.</div>';
            return;
          }
          for(const ev of events.slice(0,15)){
            const item=document.createElement("div"); item.className="acct-timeline-item";
            const dot=document.createElement("div"); dot.className="acct-timeline-dot"; dot.style.background=ev.color;
            const content=document.createElement("div"); content.className="acct-timeline-content";
            const titleEl=document.createElement("div"); titleEl.className="acct-timeline-title"; titleEl.textContent=ev.title;
            const meta=document.createElement("div"); meta.className="acct-timeline-meta"; meta.textContent=ev.date?fmtDate(ev.date):"";
            content.appendChild(titleEl); content.appendChild(meta);
            if(ev.body){ const body=document.createElement("div"); body.className="acct-timeline-body"; body.textContent=ev.body.slice(0,200); content.appendChild(body); }
            item.appendChild(dot); item.appendChild(content); detailBody.appendChild(item);
          }
        }

        else if(tabKey==="deals"){
          if(!c.relDeals.length){
            detailBody.innerHTML='<div style="font-size:10px;color:var(--text-dim);padding:10px 0;">No deals found for this account.</div>';
            return;
          }
          ...
          if(open.length){
            const h=document.createElement("div"); h.className="acct-section-label"; h.textContent="OPEN ("+open.length+")"; detailBody.appendChild(h);
            open.forEach(d=>detailBody.appendChild(dealRow(d)));
          }
          if(closed.length){
            const h=document.createElement("div"); h.className="acct-section-label"; h.style.marginTop="12px"; h.textContent="CLOSED ("+closed.length+")"; detailBody.appendChild(h);
            closed.forEach(d=>detailBody.appendChild(dealRow(d)));
          }
        }

        else if(tabKey==="issues"){
          if(!c.relIssues.length){
            detailBody.innerHTML='<div style="font-size:10px;color:var(--text-dim);padding:10px 0;">No related Linear issues found. Issues are matched when the initiative or project name contains "'+c.name+'".</div>';
            return;
          }
          for(const iss of c.relIssues.sort((a,b)=>{const r={Urgent:0,High:1,Medium:2,Low:3};return (r[a.priority]??4)-(r[b.priority]??4);})){
            ...
            const ttl=document.createElement("span"); ttl.style.cssText="flex:1;color:var(--text);"; ttl.textContent=iss.title;
            const pri=document.createElement("span");
            const priCls=priorityLevel(iss.priority);
            pri.style.cssText=\`font-size:8px;padding:1px 5px;border-radius:3px;flex-shrink:0;color:\${priCls==="urgent"?"#ef4444":priCls==="high"?"#f97316":"var(--text-dim)"};\`;
            pri.textContent=iss.priority||"";
            const assignee=document.createElement("span"); assignee.style.cssText="font-size:8px;color:var(--text-dim);flex-shrink:0;"; assignee.textContent=iss.assignee?iss.assignee.split(" ")[0]:"";
            row.appendChild(id); row.appendChild(statusDot); row.appendChild(ttl); row.appendChild(pri); row.appendChild(assignee);
      ...
      for(const td of tabDefs){
        const tabBtn=document.createElement("button"); tabBtn.className="acct-detail-tab"+(td.key==="plan"?" active":""); tabBtn.dataset.tab=td.key;
        tabBtn.textContent=td.label;
        tabBtn.addEventListener("click",(e)=>{ e.stopPropagation(); renderDetailTab(td.key); });
        detailTabs.appendChild(tabBtn);
      }
      // HubSpot link
      if(portalId){
        const hsLink=document.createElement("a");
        hsLink.style.cssText="font-size:9px;color:var(--accent);margin-left:auto;padding:8px 12px;text-decoration:none;display:flex;align-items:center;gap:3px;";
        hsLink.href="https://app.hubspot.com/contacts/"+portalId+"/company/"+c.id;
        hsLink.target="_blank"; hsLink.textContent="\u2197 HubSpot";
        hsLink.addEventListener("click",e=>e.stopPropagation());
        detailTabs.appendChild(hsLink);
      }
      detail.appendChild(detailTabs);
      ...
      card.addEventListener("click",()=>{
        ...
        if(expanded) renderDetailTab(activeDetailTab);
      });
    ...
    if(!visible.length){
      const em=document.createElement("div"); em.style.cssText="padding:40px;text-align:center;color:var(--text-dim);font-size:11px;";
      em.textContent="No accounts match your search or filter."; listEl.appendChild(em);
    }
  }
...
function renderKPIs(){
  ...
  scenarioWrap.style.cssText="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:20px;margin-top:16px;";
  ...
  for(const [key,cfg] of Object.entries(inputs)){
    ...
    const lbl=document.createElement("div"); lbl.style.cssText="font-size:8px;text-transform:uppercase;letter-spacing:.8px;color:var(--text-muted);margin-bottom:6px;"; lbl.textContent=cfg.label;
    const valDisp=document.createElement("div"); valDisp.style.cssText="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:6px;"; valDisp.textContent=cfg.fmt(cfg.val);
    const slider=document.createElement("input"); slider.type="range"; slider.min=cfg.min; slider.max=cfg.max; slider.step=cfg.step; slider.value=cfg.val;
    slider.style.cssText="width:100%;accent-color:var(--accent);cursor:pointer;";
    slider.addEventListener("input",()=>{
      vals[key]=parseFloat(slider.value);
      valDisp.textContent=cfg.fmt(vals[key]);
      recompute();
    });
    cell.appendChild(lbl); cell.appendChild(valDisp); cell.appendChild(slider);
  ...
  outputWrap.appendChild(summaryPanel);
  outputWrap.appendChild(chartPanel);
  ...
  function recompute(){
    ...
    const runwayMonth=projections.findIndex(p=>p.cash<=0);
    const runwayMonths=runwayMonth===-1?">"+months:String(runwayMonth+1);
    const endCash=projections[projections.length-1].cash;
    const breakEvenMonth=projections.findIndex(p=>p.net>=0);
    const breakEvenStr=breakEvenMonth===-1?"Not in "+months+"mo":"Month "+(breakEvenMonth+1);

    // Summary
    summaryPanel.innerHTML=\`
      <div style="font-size:10px;font-weight:600;color:var(--text);margin-bottom:12px;">\u{1F4CA} Projected Outcomes</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div style="text-align:center;padding:10px;background:var(--bg);border-radius:6px;">
          <div style="font-size:8px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;">Runway</div>
          <div style="font-size:20px;font-weight:700;color:\${runwayMonth!==-1&&runwayMonth<12?"#ef4444":"#10b981"}">\${runwayMonths} mo</div>
        </div>
        <div style="text-align:center;padding:10px;background:var(--bg);border-radius:6px;">
          <div style="font-size:8px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;">Break Even</div>
          <div style="font-size:20px;font-weight:700;color:\${breakEvenMonth!==-1?"#10b981":"#f59e0b"}">\${breakEvenStr}</div>
        </div>
        <div style="text-align:center;padding:10px;background:var(--bg);border-radius:6px;">
          <div style="font-size:8px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;">End Cash (\${months}mo)</div>
          <div style="font-size:20px;font-weight:700;color:\${endCash>0?"#10b981":"#ef4444"}">\${cur}\${endCash>=1e6?(endCash/1e6).toFixed(1)+"M":endCash>=1e3?Math.round(endCash/1e3)+"k":endCash.toLocaleString()}</div>
        </div>
        <div style="text-align:center;padding:10px;background:var(--bg);border-radius:6px;">
          <div style="font-size:8px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;">End MRR</div>
          <div style="font-size:20px;font-weight:700;color:#3b82f6">\${cur}\${Math.round(projections[projections.length-1].mrr).toLocaleString()}</div>
        </div>
      </div>
    \`;
    ...
    for(const p of projections){
      ...
      tblHtml+=\`<tr style="border-bottom:1px solid rgba(255,255,255,.04);">
        ...
        <td style="padding:4px 6px;text-align:right;position:relative;">
          ...
          <span style="position:relative;color:\${cashColor};font-weight:600;">\${cur}\${Math.abs(p.cash)>=1e6?(p.cash/1e6).toFixed(1)+"M":Math.abs(p.cash)>=1e3?Math.round(p.cash/1e3)+"k":p.cash}</span>
        </td>
      </tr>\`;
    }
    tblHtml+='</tbody></table>';
    chartPanel.innerHTML=tblHtml;
  }

  recompute();

  // WBSO Time Tracker (admin only)
  if(isAdmin()) renderWBSOTracker(main);
}

// ═══════════════════════════════════════════════════════════════════════════
// WBSO TIME TRACKER
// ═══════════════════════════════════════════════════════════════════════════
function renderWBSOTracker(main){
  if(!WBSO_DATA||!WBSO_DATA.blocks||!WBSO_DATA.blocks.length) return;
  ...
  const currentWeek=getISOWeek(TODAY_STR);

  // State for week selector
  let selectedWeek=currentWeek;

  // Container
  const cont=document.createElement("div");
  cont.style.cssText="margin-top:24px;border-top:2px solid var(--border);padding-top:20px;";

  // Header section
  const hdr=document.createElement("div");
  hdr.style.cssText="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:16px;";
  const titleDiv=document.createElement("div");
  titleDiv.innerHTML=\`<div style="font-size:16px;font-weight:700;color:var(--text);">🧪 WBSO R&D Time Tracker</div>
    <div style="font-size:9px;color:var(--text-muted);margin-top:3px;">Application \${WBSO_DATA.application||WBSO_DATA.applicationNumber||"—"} · \${WBSO_DATA.company||""} · \${WBSO_DATA.period?WBSO_DATA.period.start+" → "+WBSO_DATA.period.end:""}</div>\`;
  const exportBtn=document.createElement("button");
  exportBtn.style.cssText="font-size:9px;padding:4px 10px;background:var(--accent);color:white;border:none;border-radius:4px;cursor:pointer;font-weight:600;";
  exportBtn.textContent="Export CSV";
  exportBtn.onclick=()=>{
    let csv="Week,Block,Issue ID,Issue Title,Hours,Notes\\n";
    const blocks=WBSO_DATA.blocks||[];
    for(const block of blocks){
      for(let w=1;w<=52;w++){
        const key=\`supplied_wbso_hours_\${w}_\${block.id}\`;
        const notesKey=\`supplied_wbso_notes_\${w}\`;
        const hours=localStorage.getItem(key)||"";
        const notes=localStorage.getItem(notesKey)||"";
        const issues=block.issues||[];
        for(const issue of issues){
          const lineHours=localStorage.getItem(\`supplied_wbso_issue_hours_\${w}_\${issue.id}\`)||"";
          csv+=\`\${w},\${block.name},\${issue.id},\${issue.title},\${lineHours},\${notes}\\n\`;
        }
        if(!issues.length&&hours){
          csv+=\`\${w},\${block.name},,\${hours},\${notes}\\n\`;
        }
      }
    }
  ...
  cardsRow.style.cssText="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:20px;";

  for(const block of WBSO_DATA.blocks){
    const allocated=block.allocatedHours||0;
    let logged=0;
    for(let w=1;w<=52;w++){
      const key=\`supplied_wbso_hours_\${w}_\${block.id}\`;
      const val=localStorage.getItem(key);
      if(val) logged+=parseFloat(val)||0;
    }
    const pct=allocated>0?Math.round((logged/allocated)*100):0;
    const isOnTrack=pct>=85;
    const isBehind=pct>=60&&pct<85;
    const isOffTrack=pct<60;
    const color=isOnTrack?"#10b981":isBehind?"#f59e0b":"#ef4444";

    const card=document.createElement("div");
    card.style.cssText=\`padding:12px;background:var(--surface);border:1px solid var(--border);border-radius:8px;border-left:3px solid \${color};\`;
    card.innerHTML=\`<div style="font-size:10px;font-weight:600;color:var(--text);">\${block.name}</div>
      <div style="font-size:11px;color:var(--text-muted);margin-top:6px;">
        <div>\${logged}h / \${allocated}h</div>
        <div style="height:4px;background:var(--surface2);border-radius:2px;margin:6px 0;position:relative;">
          <div style="height:100%;background:\${color};border-radius:2px;width:\${pct}%;"></div>
        </div>
  ...
  navRow.style.cssText="display:flex;align-items:center;gap:8px;margin-bottom:16px;overflow-x:auto;padding:8px 0;";

  const prevBtn=document.createElement("button");
  prevBtn.textContent="◀";
  prevBtn.style.cssText="background:var(--surface2);border:1px solid var(--border);padding:4px 8px;border-radius:4px;cursor:pointer;flex-shrink:0;font-size:11px;";
  prevBtn.onclick=()=>{
  ...
  weekContainer.style.cssText="display:flex;gap:4px;flex:1;justify-content:center;";

  function updateWeekView(){
    weekContainer.innerHTML="";
    const startWeek=Math.max(1,selectedWeek-4);
    const endWeek=Math.min(52,selectedWeek+3);
  ...
  navRow.appendChild(weekContainer);

  const nextBtn=document.createElement("button");
  nextBtn.textContent="▶";
  nextBtn.style.cssText="background:var(--surface2);border:1px solid var(--border);padding:4px 8px;border-radius:4px;cursor:pointer;flex-shrink:0;font-size:11px;";
  nextBtn.onclick=()=>{
    selectedWeek=Math.min(52,selectedWeek+1);
    updateWeekView();
  };
  navRow.appendChild(nextBtn);

  cont.appendChild(navRow);

  // Weekly detail panel
  const detailPanel=document.createElement("div");
  detailPanel.style.cssText="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px;margin-bottom:20px;";

  function updateDetailPanel(){
    detailPanel.innerHTML="";
    const wr=getWeekRange(currentYear,selectedWeek);
    const wStart=wr.start.toISOString().split("T")[0];
    const wEnd=wr.end.toISOString().split("T")[0];
    const fmtStart=wr.start.toLocaleDateString("en-GB",{day:"2-digit",month:"short"});
    const fmtEnd=wr.end.toLocaleDateString("en-GB",{day:"2-digit",month:"short"});

    const weekHdr=document.createElement("div");
    weekHdr.style.cssText="font-size:12px;font-weight:700;color:var(--text);margin-bottom:12px;";
    weekHdr.textContent=\`\${currentYear}-W\${String(selectedWeek).padStart(2,"0")} (\${fmtStart} – \${fmtEnd})\`;
    detailPanel.appendChild(weekHdr);
    ...
    for(const block of WBSO_DATA.blocks){
      ...
      blockRow.style.cssText="border-bottom:1px solid var(--border);";
      const blockCell=document.createElement("td");
      blockCell.colSpan="4";
      blockCell.style.cssText="padding:8px 6px;background:var(--surface2);font-weight:700;color:var(--text);";

      const blockHours=parseFloat(localStorage.getItem(\`supplied_wbso_hours_\${selectedWeek}_\${block.id}\`))||0;
      blockCell.textContent=\`\${block.name} — \${blockHours}h\`;
      blockRow.appendChild(blockCell);
      tbl.appendChild(blockRow);

      const issues=block.issues||[];
      for(const issue of issues){
        const issueRow=document.createElement("tr");
        issueRow.style.cssText="border-bottom:1px solid rgba(255,255,255,.04);";

        const cbCell=document.createElement("td");
        cbCell.style.cssText="padding:6px;width:24px;text-align:center;";
        const cb=document.createElement("input");
        cb.type="checkbox";
        cb.style.cssText="cursor:pointer;";
        cbCell.appendChild(cb);
        issueRow.appendChild(cbCell);

        const idCell=document.createElement("td");
        idCell.style.cssText="padding:6px;color:var(--accent);";
        const issueLink=document.createElement("a");
        issueLink.href=issue.linearUrl||"#";
        issueLink.textContent=issue.id;
        issueLink.style.cssText="text-decoration:none;color:var(--accent);";
        issueLink.target="_blank";
        issueLink.rel="noopener";
        idCell.appendChild(issueLink);
        issueRow.appendChild(idCell);

        const titleCell=document.createElement("td");
        titleCell.style.cssText="padding:6px;color:var(--text);flex:1;";
        titleCell.textContent=issue.title;
        issueRow.appendChild(titleCell);

        const hoursCell=document.createElement("td");
        hoursCell.style.cssText="padding:6px;text-align:right;width:50px;";
        const hoursInput=document.createElement("input");
        hoursInput.type="number";
        hoursInput.min="0";
        hoursInput.step="0.5";
        hoursInput.style.cssText="width:100%;padding:2px 4px;font-size:9px;border:1px solid var(--border);border-radius:3px;background:var(--surface2);color:var(--text);";
        const hKey=\`supplied_wbso_issue_hours_\${selectedWeek}_\${issue.id}\`;
        hoursInput.value=localStorage.getItem(hKey)||"";
        hoursInput.onchange=()=>{
          const val=parseFloat(hoursInput.value)||0;
          if(val>0){
            localStorage.setItem(hKey,String(val));
          } else {
        ...
        issueRow.appendChild(hoursCell);

        tbl.appendChild(issueRow);
      }
    }

    // Total row
    const totalRow=document.createElement("tr");
    totalRow.style.cssText="border-top:2px solid var(--border);font-weight:700;color:var(--text);";
    const totalLabelCell=document.createElement("td");
    totalLabelCell.colSpan="3";
    totalLabelCell.style.cssText="padding:8px 6px;text-align:right;";
    totalLabelCell.textContent="Total:";
    totalRow.appendChild(totalLabelCell);

    const totalValCell=document.createElement("td");
    totalValCell.style.cssText="padding:8px 6px;text-align:right;";
    let totalHours=0;
    for(const block of WBSO_DATA.blocks){
      for(const issue of (block.issues||[])){
        const hKey=\`supplied_wbso_issue_hours_\${selectedWeek}_\${issue.id}\`;
        const val=localStorage.getItem(hKey);
        if(val) totalHours+=parseFloat(val)||0;
      }
    }
    totalValCell.textContent=\`\${totalHours}h\`;
    totalRow.appendChild(totalValCell);
    ...
    notesLbl.style.cssText="font-size:10px;font-weight:600;color:var(--text);margin-top:16px;margin-bottom:6px;";
    notesLbl.textContent="Weekly Notes:";
    detailPanel.appendChild(notesLbl);

    const notesTA=document.createElement("textarea");
    notesTA.style.cssText="width:100%;min-height:80px;padding:6px 8px;font-size:9px;border:1px solid var(--border);border-radius:4px;background:var(--surface2);color:var(--text);font-family:inherit;resize:vertical;";
    const notesKey=\`supplied_wbso_notes_\${selectedWeek}\`;
    notesTA.value=localStorage.getItem(notesKey)||"";
    notesTA.onchange=()=>{
      if(notesTA.value){
        localStorage.setItem(notesKey,notesTA.value);
      } else {
        localStorage.removeItem(notesKey);
      }
    };
    detailPanel.appendChild(notesTA);
  }

  updateWeekView();
  updateDetailPanel();

  cont.appendChild(detailPanel);
...
function renderMeetingSummaries(container){
  ...
  for(const mtg of meetings){
    ...
    if(mtg.suggestedTasks&&mtg.suggestedTasks.length){
      ...
      const MTG_DONE_KEY="supplied_mtg_done";
      const mtgDone=new Set(JSON.parse(localStorage.getItem(MTG_DONE_KEY)||"[]"));
      function saveMtgDone(){ localStorage.setItem(MTG_DONE_KEY, JSON.stringify([...mtgDone])); }

      for(const st of mtg.suggestedTasks){
        const taskId="mtg-"+mtg.date+"-"+(st.task||"").slice(0,30).replace(/\\s+/g,"-").toLowerCase();
        const isDone=mtgDone.has(taskId);
        const stRow=document.createElement("div");
        stRow.style.cssText="display:flex;align-items:center;gap:6px;padding:4px 0;font-size:10px;border-bottom:1px solid rgba(255,255,255,.04);"+(isDone?"opacity:.4;":"");

        const cb=document.createElement("input");
        cb.type="checkbox"; cb.checked=isDone;
        cb.style.cssText="flex-shrink:0;cursor:pointer;accent-color:var(--accent);width:13px;height:13px;";

        const stName=document.createElement("span");
        stName.style.cssText="color:var(--text);flex:1;line-height:1.4;"+(isDone?"text-decoration:line-through;":"");
        stName.textContent=st.task;

        cb.addEventListener("change",function(){
          if(cb.checked){ mtgDone.add(taskId); stRow.style.opacity=".4"; stName.style.textDecoration="line-through"; }
          else { mtgDone.delete(taskId); stRow.style.opacity=""; stName.style.textDecoration=""; }
          saveMtgDone();
          // Update badge count
          const remaining=mtg.suggestedTasks.filter(t=>!mtgDone.has("mtg-"+mtg.date+"-"+(t.task||"").slice(0,30).replace(/\\s+/g,"-").toLowerCase())).length;
          taskBadge.textContent=remaining+" tasks";
        });
...
function renderMyDay(){
  ...
  if(showPipeline && HS_DATA && HS_DATA.deals && HS_DATA.deals.length){
    ...
    for(const d of openDeals){
      const sl=d.stageLabel||d.stage||"Unknown";
      if(!stageDeals[sl]) stageDeals[sl]={count:0,value:0,deals:[]};
      stageDeals[sl].count++;
      stageDeals[sl].value+=(d.amount||0);
      stageDeals[sl].deals.push(d);
    }
    const stages=orderedStages.length
      ? orderedStages.filter(s=>stageDeals[s.label]).map(s=>({label:s.label,...stageDeals[s.label]}))
      : Object.entries(stageDeals).map(([label,v])=>({label,...v}));

    if(stages.length){
      const sideHdr = document.createElement("div");
      sideHdr.className = "myday-sidebar-hdr";
      sideHdr.innerHTML=\`\uD83C\uDFC6 PIPELINE <span style="float:right;font-size:9px;color:var(--accent)">\${cur}\${totalPipeVal.toLocaleString()}</span>\`;
      sidebar.appendChild(sideHdr);

      const STAGE_COLORS=["#f59e0b","#22c55e","#3b82f6","#8b5cf6","#ef4444","#06b6d4","#ec4899","#64748b"];
      stages.forEach((stg,si)=>{
        // Stage header row
        const stgRow=document.createElement("div");
        stgRow.style.cssText="display:flex;align-items:center;gap:6px;padding:4px 12px;cursor:pointer;user-select:none;";
        const dot=document.createElement("span");
        ...
        const stgLabel=document.createElement("span");
        stgLabel.style.cssText="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;flex:1;";
        stgLabel.textContent=stg.label+" ("+stg.count+")";
        const stgVal=document.createElement("span");
        stgVal.style.cssText=\`font-size:9px;color:\${c};font-weight:600;\`;
        stgVal.textContent=cur+stg.value.toLocaleString();
        const chevron=document.createElement("span");
        chevron.textContent="\u25B8";
        chevron.style.cssText="font-size:8px;color:var(--text-muted);transition:transform .2s;";
        stgRow.appendChild(dot); stgRow.appendChild(stgLabel); stgRow.appendChild(stgVal); stgRow.appendChild(chevron);

        // Deal rows (collapsed by default)
        const dealList=document.createElement("div");
        dealList.style.display="none";
        for(const d of stg.deals.sort((a,b)=>(b.amount||0)-(a.amount||0))){
          const dr=document.createElement("div");
          dr.className="myday-deal-row";
          dr.style.paddingLeft="22px";
          const amt=d.amount?cur+Number(d.amount).toLocaleString():"";
          const owner=(d.ownerName||"").split(" ")[0];
          const closeStr=d.closeDate?fmtDate(d.closeDate):"";
          ...
          dr.innerHTML=\`
            ...
            \${owner?'<span style="font-size:8px;color:var(--text-dim);background:rgba(255,255,255,.06);padding:1px 5px;border-radius:3px;">'+owner+'</span>':""}
            \${closeStr?'<span class="myday-deal-date">'+closeStr+'</span>':""}
          \`;
          dealList.appendChild(dr);
        }

        let expanded=false;
        stgRow.addEventListener("click",()=>{
          expanded=!expanded;
          dealList.style.display=expanded?"block":"none";
          chevron.style.transform=expanded?"rotate(90deg)":"";
        });

        sidebar.appendChild(stgRow);
        sidebar.appendChild(dealList);
      });
    }
  }
  ...
  // ── Calendar sidebar ──────────────────────────────────────────────────────
  if(showCal){
    // Re-classify events based on actual current date (data may be stale)
    const allCalEvents = [
      ...((GOOGLE_DATA&&GOOGLE_DATA.calendar&&GOOGLE_DATA.calendar.today)||[]),
      ...((GOOGLE_DATA&&GOOGLE_DATA.calendar&&GOOGLE_DATA.calendar.upcoming)||[])
    ];
    const evDate = ev => {
      const s = ev.start||ev.date||"";
      return s.includes("T") ? s.split("T")[0] : s.slice(0,10);
    };
    const todayEvents = allCalEvents.filter(ev=>evDate(ev)===TODAY_STR);
    const upcomingEvents = allCalEvents.filter(ev=>evDate(ev)>TODAY_STR);
    // Sort by start time
    ...
    if(todayEvents.length){
      ...
      noCal.style.cssText="font-size:10px;color:var(--text-dim);padding:8px 12px;";
      noCal.textContent="No events scheduled for today. Run Refresh to update calendar.";
    ...
    if(upcomingEvents.length){
      ...
      for(const ev of upcomingEvents.slice(0,8)){
        const er = document.createElement("div");
        er.className = "myday-deal-row";
...
function buildDataContext(tabKey){
  ...
  lines.push("TODAY: "+now.toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})+" | CURRENT TIME: "+nowHH);
  ...
  if(inclGoogle&&GOOGLE_DATA){
    ...
    if(cal&&(cal.today||[]).length){
      lines.push("\\nMEETINGS TODAY (current time is "+nowHH+"):");
      for(const ev of cal.today){
        // Determine if meeting is past, current, or upcoming
        let timeTag="";
        if(ev.start){
          const evStart=ev.start.replace(/[^0-9:]/g,"").slice(0,5);
          const evEnd=ev.end?ev.end.replace(/[^0-9:]/g,"").slice(0,5):null;
          if(evEnd&&nowHH>=evEnd) timeTag=" [ALREADY FINISHED]";
          else if(nowHH>=evStart) timeTag=" [HAPPENING NOW]";
          else timeTag=" [UPCOMING]";
        }
        lines.push("  "+ev.title+(ev.start?" @ "+ev.start:"")+timeTag);
      }
    }
    else lines.push("\\nMEETINGS: None today");
...
function streamClaudeReply(userMsg,silent,tabKey){
  ...
  callClaudeStream(CLAUDE_HISTORIES[tk],
    ...
    ()=>{
      CLAUDE_HISTORIES[tk].push({role:"assistant",content:full});
      // Detect [[TASK:...]] tags and add to custom tasks
      const taskMatches=full.match(/\\[\\[TASK:([^\\]]+)\\]\\]/g);
      if(taskMatches&&taskMatches.length&&typeof CUSTOM_TASKS!=="undefined"){
        for(const tm of taskMatches){
          const title=tm.replace(/^\\[\\[TASK:/,"").replace(/\\]\\]$/,"").trim();
          if(title){
            const newTask={id:"custom-"+Date.now()+"-"+Math.random().toString(36).slice(2,6),title:title,url:null};
            CUSTOM_TASKS.unshift(newTask); saveCustomTasks();
          }
        }
        // Re-render My Day to show new tasks
        if(currentTab==="myday") renderMyDay();
      }
      // Clean up [[TASK:...]] markers from displayed text
      if(textEl&&taskMatches) textEl.textContent=full.replace(/\\[\\[TASK:([^\\]]+)\\]\\]/g,"\u2705 $1");
...
async function init(){
  ...
  // ── LOGIN GATE: force user to identify before seeing anything ──────────
  if(!CURRENT_USER){
    document.body.classList.add("locked");
    // Pre-load just enough data to build the user selector
    try{
      const gr=await fetch('./gantt-data.json?t='+Date.now());
      if(gr.ok) GANTT_DATA=await gr.json();
    }catch(e){}
    try{
      const hr=await fetch('./hubspot-data.json?t='+Date.now());
      if(hr.ok) HS_DATA=await hr.json();
    }catch(e){}
    buildUserOptions();
    openUserSelector();
    if(btn) btn.classList.remove("loading");
    return; // ← stop here, don't render any dashboard content
  }
  ...
  // Load WBSO time tracker data (optional)
  try {
    const wr=await fetch('./wbso-data.json?t='+Date.now());
    if(wr.ok){ WBSO_DATA=await wr.json(); }
  } catch(e){ /* WBSO data not available */ }
  ...
  // Build user options now that data is loaded
  buildUserOptions();
  applyAccessControl();
...
// ═══════════════════════════════════════════════════════════════════════════
// AUTO-REFRESH: silently trigger GitHub Actions workflow every hour
// ═══════════════════════════════════════════════════════════════════════════
...
async function autoRefresh(){
  const token=getStoredPat();
  if(!token||!CURRENT_USER) return; // skip if no PAT or not logged in
  console.log("[auto-refresh] triggering at", new Date().toLocaleTimeString());
  const el=document.getElementById("refresh-time");
  try{
    const triggered=await triggerWorkflow(token);
    if(!triggered){ console.warn("[auto-refresh] trigger failed"); return; }
    if(el) el.textContent="AUTO-REFRESHING…";
    const done=await pollWorkflowDone(token);
    // Silently reload data without disrupting the UI
    GANTT_DATA=null; HS_DATA=null; GOOGLE_DATA=null;
    KPI_DATA=null; COST_DATA=null; WBSO_DATA=null; MTG_DATA=null;
    await init();
    console.log("[auto-refresh] complete at", new Date().toLocaleTimeString());
  }catch(e){
    console.warn("[auto-refresh] error:",e);
    if(el) el.textContent="UPDATED "+new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"});
  }
}

function startAutoRefresh(){
  if(AUTO_REFRESH_INTERVAL) clearInterval(AUTO_REFRESH_INTERVAL);
  AUTO_REFRESH_INTERVAL=setInterval(autoRefresh, AUTO_REFRESH_MS);
  console.log("[auto-refresh] scheduled every", AUTO_REFRESH_MS/60000, "min");
}
// Start auto-refresh once page loads (only runs if PAT is stored)
startAutoRefresh();
</script>
</body>
</html>`;
